...
class WebScraper:
    ...
    def extract_text(self, html: str, max_length: int = 50000) -> str:
        """提取纯文本内容"""
        try:
    ...
    async def _fetch_normal_page(self, url: str) -> Optional[Dict]:
        ...
        if not content or len(content) < 100:
            content = self.extract_text(html, max_length=50000)
    ...
    async def scrape_article(self, url: str) -> Dict:
        ...
        if not content or len(content) < 100:
            for selector in ['.content', '.main-content', '.post-content', '.entry-content', '#content']:
                container = soup.select_one(selector)
                if container:
                    content = container.get_text(separator='\n', strip=True)
                    if len(content) > 100:
